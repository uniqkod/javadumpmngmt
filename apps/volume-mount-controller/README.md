# Volume Mount Access Controller
...
The Volume Mount Access Controller runs as a privileged StatefulSet in Kubernetes/OpenShift, managing access permissions for shared volume mounts across multiple applications running with different user IDs.
...
```bash
# Create secret
kubectl apply -f k8s/apps/volume-mount-controller/secret.yaml

# Deploy StatefulSet and Service
kubectl apply -f k8s/apps/volume-mount-controller/statefulset.yaml
kubectl apply -f k8s/apps/volume-mount-controller/service.yaml
```

### Deploy to OpenShift
```bash
# Create ImageStream and BuildConfig
oc apply -f openshift/buildconfigs/volume-mount-controller-is.yaml
oc apply -f openshift/buildconfigs/volume-mount-controller-bc.yaml

# Start build
oc start-build volume-mount-controller
...
# Create Route
oc apply -f openshift/routes/volume-mount-controller.yaml
```
...
### Register a Mount
```bash
curl -X POST https://volume-mount-controller.apps.example.com/api/v1/app/mount/register \
...
```bash
curl -X POST https://volume-mount-controller.apps.example.com/api/v1/app/mount/ready \
  -H "X-API-Key: your-api-key" \
  -H "Content-Type: application/json" \
  -d '{
    "appName": "my-app",
...
```bash
java -jar target/volume-mount-controller-1.0.0.jar \
  --api.key=test-key \
...
### Check Pod Status
```bash
kubectl get pods -l app=volume-mount-controller -n heapdump
kubectl logs -l app=volume-mount-controller -n heapdump
```

### Verify API Key
```bash
kubectl get secret volume-mount-api-key -n heapdump -o yaml
```

### Test Health Endpoints
```bash
kubectl exec -it volume-mount-controller-0 -n heapdump -- curl localhost:8080/api/v1/health
```
